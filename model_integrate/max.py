...
class Integrate(nn.Module):
    ...
    def forward(self, mes_update, yv, structure, *args, **kargs):
        ...
        cask_mask_enlarge = cas_mask.unsqueeze(-1).expand(-1, -1, cas_all.size(-1))
        cas_all_masked = cas_all.masked_fill_(cask_mask_enlarge==0, -1e9)
        cas_max = cas_all.max(dim=1)[0] # (batch_cas, node_size)
        yv_cas = yv[source_index]
        return cas_max, yv_cas
